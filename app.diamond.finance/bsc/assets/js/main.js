...
Additional Rewards - No`,
    ...
    methods: {
        dataLoad: async function() {
            ...
            scr4.src = "api.diam.finance/realtimeData.js" + "?ts=" + new Date().getTime();
            ...
            scr5.src = "api.diam.finance/vaultsData.js" + "?ts=" + new Date().getTime();
            ...
            scr8.src = " api.diam.finance/farmsData.js" + "?ts=" + new Date().getTime();
            ...
            scr9.src = " api.diam.finance/wpoolsData.js" + "?ts=" + new Date().getTime();
            ...
            this.DiAMLpPrice = realtimeData.diamxLpPrice;
            this.DIAMCirculation = realtimeData.DIAMCirculation;
            this.DIAMBurned = realtimeData.DIAMBurned;
            this.stakingPools[4].apy = realtimeData.DiAMApy;
            this.stakingPools[4].totalValueLocked = realtimeData.DiAMTvl;
            //diam
            this.DIAMTotalValueLocked = realtimeData.DIAMTotalValueLocked;
        ...
        fetchData: async function() {
            ...
            this.diamTokenContract = new this.web3.eth.Contract(diamAbi, this.diamTokenAddress);
            this.diamLpContract = new this.web3.eth.Contract(lpAbi, this.diamLpAddress);
            ...
            if (this.selectedSection == 0) {
                try {
                    ...
                    let temp = await this.diamTokenContract.methods.allowance(this.selectedAccount, this.DiAMSwapAddress).call();
            ...
            if (this.selectedSection == 5) {
                try {
                    ...
                    let temp = await this.diamTokenContract.methods.allowance(this.selectedAccount, this.stakingAddress).call();
                    this.stakingApproved = this.web3.utils.fromWei(temp) >= this.web3.utils.fromWei('1' + (Array(78).join('0')));
                    for (let i = 1; i < 4; i++) {
                        this.stakingPools[i].balance = this.diamBalance;
                    ...
                    this.lpBalance = await this.diamLpContract.methods.balanceOf(this.selectedAccount).call();
                    this.lpBalance = math.format(Math.floor(Number(this.web3.utils.fromWei(this.lpBalance)) * Math.pow(10, 4)) / Math.pow(10, 4), {
                        notation: 'fixed'
                    });
                    temp = await this.diamLpContract.methods.allowance(this.selectedAccount, this.lm2Address).call();
            ...
            if (this.selectedSection == 6) {
                for (let i = 0; i < this.vaults.length; i++) {
                    ...
                    if (this.vaults[i].id === 'ellipsis-3eps')
                        ...
                        this.vaults[i].additionalRewards = (await this.vaults[i].vaultContract.methods.balanceOfRewards(this.selectedAccount).call())._diams;
        ...
        DiAMSwapApprove: async function() {
            try {
                await this.diamTokenContract.methods.approve(this.DiAMSwapAddress, '1' + (Array(78).join('0'))).send({
        ...
        lm2Approve: async function(stake) {
            try {
                await this.diamLpContract.methods.approve(this.lm2Address, '1' + (Array(78).join('0'))).send({
                    'from': this.selectedAccount
                });
                this.fetchData();
            } catch (e) {
        ...
        stakeApprove: async function(stake) {
            ...
            try {
                if (stake.i == 4) {
                    ...
                    await this.diamTokenContract.methods.approve(this.stakingAddress, '1' + (Array(78).join('0'))).send({
                        'from': this.selectedAccount
                    });
                }
                this.fetchData();
